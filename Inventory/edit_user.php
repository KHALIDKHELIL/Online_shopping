...
?>
<?php
$e_user = find_by_id('admin', (int)$_GET['id']);
...
if (isset($_POST['update'])) {
  ...
  if (empty($errors)) {
    ...
    if ($id === (int)$_SESSION['user_id'] && $status === 0) {
      $session->msg('d', "You cannot deactivate your own account.");
      redirect('edit_user.php?id=' . (int)$e_user['id'], false);
    }

    if ( $level === $_SESSION['user_level']) {
      $session->msg('d', "You cannot edit account.");
      redirect('edit_user.php?id=' . (int)$e_user['id'], false);
    }

    $sql = "UPDATE admin SET name='{$name}', username='{$username}', user_level='{$level}', status='{$status}' WHERE id='{$db->escape($id)}'";
    $result = $db->query($sql);
...
if (isset($_POST['update-pass'])) {
  ...
  if (empty($errors)) {
    ...
    $sql = "UPDATE admin SET password='{$h_pass}' WHERE id='{$db->escape($id)}'";
    $result = $db->query($sql);
